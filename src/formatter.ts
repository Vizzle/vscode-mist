...
import {TextDocument, Range, Position, FormattingOptions, TextEdit} from 'vscode';
...
export function format(document: TextDocument, range: Range, options: FormattingOptions): TextEdit[] {
	...
	if (range) {
		...
		let startPosition = new Position(range.start.line, 0);
		formatTextStart = document.offsetAt(startPosition);

		let endOffset = document.offsetAt(new Position(range.end.line + 1, 0));
		let endLineStart = document.offsetAt(new Position(range.end.line, 0));
		...
		range = new Range(new Position(0, 0), document.positionAt(formatText.length));
...
function getEOL(document: TextDocument): string {
	...
	if (document.lineCount > 1) {
		let to = document.offsetAt(new Position(1, 0));
		let from = to;
